...
__all__ = ['RewardFunction', 'Reward', 'RewardModification', 'NoveltyReward', 'ContrastiveReward']
...
class RewardFunction():
    def __init__(self, weight=1, bs=None, log=True):
        self.weight = weight
        self.bs = bs
        self.score_log = {}
        self.log = log
...
class ContrastiveReward(Reward):
    ...
    def compute_and_clean(self, samples):
        rewards = self.base_reward.reward_function(samples)
        if isinstance(rewards, torch.Tensor):
            rewards = rewards.detach().cpu()
    ...
    def compute_reward(self):
        ...
        rewards = self._compute_reward(self, samples)
