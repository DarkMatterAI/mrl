...
__all__ = ['to_mol', 'to_smile', 'to_smart', 'to_mols', 'to_smiles', 'to_smarts', 'canon_smile', 'neutralize_atoms',
           'initialize_neutralisation_reactions', 'neutraize_charges', 'tokenize_smile', 'tokenize_mapped_smile',
           'molwt', 'hbd', 'hba', 'tpsa', 'rotbond', 'fsp3', 'logp', 'rings', 'heteroatoms', 'all_atoms', 'heavy_atoms',
           'formal_charge', 'molar_refractivity', 'aromaticrings', 'qed', 'sa_score', 'Catalog', 'SmartsCatalog',
           'ParamsCatalog', 'PAINSCatalog', 'PAINSACatalog', 'PAINSBCatalog', 'PAINSCCatalog', 'ZINCCatalog',
           'BRENKCatalog', 'morgan_fp', 'ECFP4', 'ECFP6', 'FCFP4', 'FCFP6', 'fp_to_array', 'tanimoto', 'tanimoto_rd',
...
def neutraize_charges(mol, reactions=None):
    'github.com/kaiwang0112006/clusfps/blob/master/third_party_package/RDKit_2015_03_1/Docs/Book/Cookbook.rst'
    mol = to_mol(mol)
...
def tokenize_smile(smile):
    'Tokenize smile by character'
    smile = to_smile(smile)
    return [i for i in smile]

def tokenize_mapped_smile(smile):
    'Tokenize smile with map tokens of the form `[{isotope}*:{map_num}]`'

    smile = to_smile(smile)
    pattern = re.compile('\[.\*:.]')
    matches = pattern.findall(smile)

    match_dict = {}
    for i, match in enumerate(matches):
        smile = smile.replace(match, f'__match_{i}__')
        match_dict[f'match_{i}'] = match

    smile = smile.split('__')
    tokens = []

    for chunk in smile:
        if chunk in match_dict.keys():
            tokens.append(match_dict[chunk])
        else:
