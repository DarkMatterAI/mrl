...
__all__ = ['Agent', 'PredictiveAgent', 'BaselineAgent', 'CriticAgent', 'GenerativeAgent']
...
class GenerativeAgent(BaselineAgent):
    def __init__(self, model, vocab, loss_function, dataset, contrastive=False,
    ...
    def after_sample(self):
        ...
        batch_ds = agent.dataset.new(sequences)
        batch = batch_ds.collate_function([batch_ds[i] for i in range(len(batch_ds))])
        batch = to_device(batch)
        bs = len(batch_ds)
        x,y = batch
...
#                 latents = self.agent.latents[latent_idxs]
    ...
    def get_model_outputs(self):
        ...
        batch_state.y_gumbel = F.one_hot(y, len(self.agent.vocab.itos)) + mprob - mprob.detach()
